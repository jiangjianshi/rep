...
@Service("buildprominService")
public class BuildProMinServiceImpl implements BuildProMinService {
	...
	public Map save(HttpServletRequest request) throws Exception {
		...
		int n = commonManager.saveOrUpdate("build_pro_min-sql", "addObj",paramMap);
		map.put("n", n);
		return map;
	}
	...
	public Map update(HttpServletRequest request) throws Exception {
		Map map = new HashMap(2);// 页面输出信息
		Map paramMap = CtrlUtils.getReqParametersMap_Utf8(request);
		int n = commonManager.saveOrUpdate("build_pro_min-sql", "updateObj",paramMap);
		map.put("n", n);
		return map;
	}
	...
	public void del(String delId) throws Exception{
		...
		commonManager.saveOrUpdate("build_pro_min-sql", "delObj", conditions);
	}
