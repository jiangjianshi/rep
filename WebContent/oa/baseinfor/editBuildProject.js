...
function save() {
	if ($('#submitForm').form('validate')) {
		var url = basePath + '/oa/baseinfor/editBuildProjectSave.c';
...
//保存
function approvalInfor_save() {
	var url = basePath + '/oa/baseinfor/approvalInforSave.c';
	$('#approvalInfor_submitForm').form('submit', {
		url : url,
		success : function(result) {
			var result = eval('(' + result + ')');
			if (result.isSuccess) {
