...
<form name="form1" method="post" action="imgDialogUpload_pros.asp" enctype="multipart/form-data">
...
Function GetFileList()
	...
	For Each o_File In o_Files
		...
		%>
		<a ondblclick="top.mainFrame().setProPic('<%=s_FileName%>');ownerDialog.close();"><img src="<%=s_CurrDir&s_FileName%>"/></a>
		<%
		end if
	Next
